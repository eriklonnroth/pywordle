...
class TileAnimation:
    ...
    def draw(self, screen, font):
        ...
        rect = pygame.Rect(self.x, self.y + y_offset, self.size, scaled_height)
...
class WordleGame:
    def __init__(self, screen):
        ...
        self.toast_message = ""
        self.letters_state = [
            [None for _ in range(GRID_SIZE)] for _ in range(NUM_ATTEMPTS)
        ]
    ...
    def load_valid_words(self) -> set:
        ...
        file_path = os.path.join(script_dir, "words.csv")
        ...
        try:
            ...
            print(f"Error: words.csv not found in {script_dir}")
            pygame.quit()
            exit(1)

        if not valid_words:
            print("Error: No valid words loaded from words.csv")
            pygame.quit()
            exit(1)
    ...
    def handle_keydown(self, event):
        if self.game_over or (
            self.current_animation and not self.current_animation.completed
        ):
            return
    ...
    def draw(self):
        ...
        for row in range(self.current_attempt):
            if (
                ...
                and not self.current_animation.completed
            ):
        ...
        for row in range(self.current_attempt, NUM_ATTEMPTS):
            jiggle_offset = (
                self.calculate_jiggle_offset() if row == self.current_attempt else 0
            )
        ...
        if self.show_toast:
            ...
            text_rect = button_text.get_rect(
                center=(WINDOW_SIZE / 2, button_y + button_height / 2)
            )
    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                button_y = (
                    GRID_OFFSET_Y
                    + (NUM_ATTEMPTS * (SQUARE_SIZE + SQUARE_SPACING))
                    + TOAST_SPACING * 1.5
                )
